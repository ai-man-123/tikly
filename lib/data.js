...
function expandUrl(tiktok_url) {
 ...
 /* require("request")({
    ...
    (err, response, body) => {
        if (err) {
            console.log(err)
            reject(new Error(`Error: Video not found / Link Invalid`))
        } else {
   ...
   try {
   resolve(( exp = require("child_process").execSync(`curl -I ${tiktok_url}`).toString());exp.includes("Location:") ? exp.split("Location: ")[1].split("\n")[0].replace("Location: ", "") : tiktok_url)
  } catch (e) {
  try {
      let form = new URLSearchParams();
      form.append("s", tiktok_url);
      let html = await (
        await axios("https://urlex.org/", { method: "POST", body: form })
      ).data;
      let $ = cheerio.load(html);
      let real = $("div.c").find("td > a").attr("href");
      if (tiktok_url == real) return reject(new Error(`Error: Video not found / Link Invalid`))
      resolve(real);
    } catch {
      reject(new Error(`Error: Video not found / Link Invalid`))
    }
